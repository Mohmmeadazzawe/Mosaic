...
import { getActivitiesPaginated, type ActivityItem, type ActivityTag } from '@/lib/api/activities';
        ...
        {activity.publishing_date && (
          <div className="flex items-center gap-2 text-gray-600 mb-3">
            ...
            <span className="text-sm">
              {locale === 'ar' ? 'تاريخ النشر:' : 'Publishing Date:'} {formatDate(activity.publishing_date)}
            </span>
          </div>
        )}
...
export default function ActivitiesPage({ params }: { params: { locale: Locale } }) {
  ...
  useEffect(() => {
    async function fetchActivities() {
      setLoading(true);
      try {
        const data = await getActivitiesPaginated(currentPage, '', '', '', '', 6, locale);
  ...
  const filteredActivities = useMemo(() => {
    ...
    if (selectedTag) {
      filtered = filtered.filter((activity) =>
        activity.tags?.some((tag) => tag.name === selectedTag)
      );
    }
    ...
    return filtered;
  }, [allActivities, selectedTag, searchQuery]);
  ...
  return (
    <div className="container mx-auto px-[3%] md:px-[4%] py-8 md:py-12">
      ...
      <div className="flex flex-col lg:flex-row gap-6 md:gap-8">
        {/* Column 1: Main Content - Activities */}
          ...
          {(filteredActivities.length > 0 || activities.length > 0) ? (
            <>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 mb-8 md:mb-12">
                {(filteredActivities.length > 0 ? filteredActivities : activities).map((activity) => (
                  <ActivityCard
                    key={activity.id}
                    activity={activity}
                    locale={locale}
              ...
              {!selectedTag && !searchQuery.trim() && (
                <div className="text-center mt-6 text-gray-600">
                  {locale === 'ar'
                    ? `عرض ${activities.length} من ${total} نشاط`
            ...
            <div className="text-center py-12">
              <p className="text-gray-500 text-lg">
                {locale === 'ar' ? 'لا توجد أنشطة متاحة' : 'No activities available'}
              </p>
            </div>
          )}
        </div>
